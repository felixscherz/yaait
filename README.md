...
For shell and PowerShell installer installations:

```sh
yaait update                    # Install the latest stable release
yaait update --version 0.2.4     # Install a specific release, including a downgrade
yaait update --check            # Check without installing
yaait --format json update --check
```

Versions may include a leading `v`. Prereleases require an explicit version.
The updater verifies the release archive's SHA-256 checksum and checks the
new executable's version before replacing the running copy. Tracker settings,
credentials, and usage caches are preserved. Downgrading the executable does
not migrate application data to an older schema.

Self-updating requires a cargo-dist installation receipt that matches the
running executable. Homebrew installations must use
`brew upgrade felixscherz/tap/yaait`. Source builds and manually downloaded
binaries must use their original installation method or be reinstalled using
the shell or PowerShell installer. `--check` works for these installations too.

JSON uses the existing response envelope with command `update`. Its data
contains `current_version`, `target_version`, `installation_method`,
`update_available`, `updated`, and `message`. Installation methods are
`installer`, `homebrew`, or `unmanaged`. `update_available` indicates that the
requested version differs from the running version, or is newer for a default
latest-release check. Checks and successful updates exit with 0; failures exit
with 1. Receipt refresh failures after replacement are reported as warnings.
...
The default output is human-readable text with used amounts and percentages.
When only a remaining amount is known, it is shown as remaining. Pass `--format json` for one
